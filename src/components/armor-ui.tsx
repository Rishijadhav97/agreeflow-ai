...
import type { DealStatus } from "@/types/armor";
...
export function Card({children,className,onClick}: {children:ReactNode;className?:string;onClick?:()=>void}){return <div className={cn("surface-card",onClick&&"surface-card--interactive",className)} onClick={onClick}>{children}</div>}
export function StatusBadge({status}:{status:DealStatus}){const key=status.toLowerCase().replaceAll(" ","-");return <span className={`status status--${key}`}><span className="status__dot"/>{status}</span>}
